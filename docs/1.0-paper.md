...
Our model performs better relative to the 4 million parameters EfficientNetB0 model [@10.48550/arXiv.1905.11946, pg. 7] evaluated by the authors of DiaMOS [@10.3390/agronomy11112107, pg. 6] with an accuracy of 86.05% and F1 score of 0.82 trained for both disease detection and symptom identification.

PlantDoc authors @Singh_2020 [pg. 3] used a pretrained InceptionResNet V2 [@https://doi.org/10.48550/arXiv.1602.07261] with a performance 70.53% accuracy and 0.70 F1 score. Our model performs better on the PlantDoc dataset with a superior 75.69% accuracy and 0.7692 F1 score. Our model generalized better when incorporating other datasets to train a model.

On par with GoogleNET, a network with 6.8 million parameters with an accuracy of 99% trained for the same task [@10.1109/CVPR.2015.7298594, pg. 6; @10.3389/fpls.2016.01419, pg. 7]. Another similar methodology used a hybrid of a convolutional network and a support vector machine that performs with 99% accuracy [@10.3389/fpls.2024.1412988, pg. 15].
...
Performance comparisons were conducted against prior work rather than evaluating state-of-the-art (SOTA) models on the MegaPlant dataset. Consequently, this limitation may affect the overall integrity of the study and should be addressed in future work.
...
![MegaPlant performance](images/results/model_performance_on_megaplant_and_foundational_datasets.png)

Performance of 3 task-specific models on the MegaPlant dataset and its three foundational datasets.
...
Future work may consider integrating additional datasets, enhancing preprocessing steps, and applying data augmentation, (i.e image rotation) to foundational datasets. Additionally, exploring alternative class normalization strategies beyond the current 12 symptom classes, ideally in consultation with professional plant pathologists.
